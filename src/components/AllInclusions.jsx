...
import ViewInclusionModal from "./ViewInclusionModal";
// const fetchURL = "http://localhost:5000";
const fetchURL='https://next-travel-backend-vercel.vercel.app'

export default function AllInclusions() {
  ...
  const fetchData = async (token) => {
    try {
      const result = await fetch(`${fetchURL}/api/v1/admin/getallinclusions`, {
        headers: {
    ...
    try {
      ...
      if (window.confirm("Are you sure you want to delete this category?")) {
        const response = await fetch(
          `${fetchURL}/api/v1/admin/deleteinclusions`,
          {
            method: "DELETE",
            headers: {
