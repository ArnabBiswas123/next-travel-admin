...
export default function ViewExclusionModal({openModal, onCloseModal,    parentfetchAgain, items}) {
...
const router=useRouter();
 ...
 const submitHandler=async()=>{
  try{
    let newErrors = {};
      if (name === "") newErrors.name = "Enter a Exclusion name";
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      const response = await fetch(`${fetchURL}/api/v1/admin/editexclusions`, {
        method: "PUT",
        headers: {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        >
          <label className=" font-home ">Exclusion Status</label>
          <select
            id="status"
            value={status}
            onChange={(e) => {
